...
- **NET-vs-TOP-HUMANS comparison (the corpus payoff):** our net (warm r2 on the new game) agrees with top BGA CoB players **~43%** of their decisions (599 decisions, 11 games) — **essentially identical to the `ai.py` heuristic (42%)**. Since the far-stronger net did NOT agree more, ~43% is a **move-diversity ceiling, not a strength gap** (biggest style gaps: `take_hex` ~19%, `take_workers`). Tooling: `cob_analyze.py` (heuristic), `cob_analyze_net.py` (net via the `move_server_coc` bridge + `cob_replay.py`'s `on_move` hook).
- **The CoB BGA-mining corpus is QUOTA-LIMITED (the one thing NOT done):** free BGA caps replays at ~10-16/day ("You have reached a limit (replay)"), so only **12 winner-preserved games** so far. The 936-net can now INGEST 4-animal corpus positions, so TRAINING on expert rows is a "get BGA Premium → scale to hundreds → harvest + fine-tune the warm net" away — but unstarted. Replay gap: some depot monasteries have `effect_id=None` (never revealed in the log) → `compact.project` maps them to a benign placeholder. All mining/analysis tooling + the coc-core port live on the **`cob-mining`** branch.
