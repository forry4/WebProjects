...
- **#6's icon shows the CURRENTLY-LIVE rule (2 workers → a building tile), NOT the new silver→2-workers rule** — the new rule is in the deferred backend batch below; shipping the new-rule icon would contradict live gameplay/tooltip. A code comment on #6 records the icon to flip to when the backend ships: `mCoin(6.4,13,1.0)+mArrowR(10.6,13,0.78)+mPawn(15,13,1.0)+mPawn(18.6,13,1.0)`.
...
- **DEFERRED backend batch (in `forrestm_projects-cobmining`, NOT shipped — would break the Expert AI):** `engine.py`/`tiles.py` mon6 rule → "spend 1 silver → gain 2 workers (no limit)" (was 2 workers → take a building tile); `tiles.py` 4th livestock animal ("chicken", 4 animals → 28 livestock tiles, 164 grand total preserved); `board.py` boards 2 & 4 rebuilt to the BGA 2019-edition layouts. These change the game the Expert net + Rust `coc-core` engine were trained on (mon6/boards/livestock all feed the net + parity tables), so they need a **Rust-table regen (`tools/gen_board_tables.py`) + net retrain** before prod, else Expert play breaks on boards 2/4 + mon6 (server/client rule+board mismatch). When shipped: also flip #6's icon and fix the stale mon6 frontend interaction (`m6Armed` workers→building flow doesn't match the new silver→2workers rule).
