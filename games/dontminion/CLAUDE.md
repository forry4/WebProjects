...
| B9 | **The 2025 Duration rule**: "the future effects of a played Duration STOP if the card fails to be in play" | Not implemented — a Duration's registered fx run at the next turn start whether or not the card survived | **⚠ THE PH.-10 AUDIT REWROTE THIS ROW: the reachability is now DOCUMENTED, not hypothetical, and the previous reasoning was wrong.** The row used to say "revisit when a set makes a Duration's death routine rather than an accident", and the ph.-10 planning read said Ways could not do it (they REPLACE the ability, so nothing is registered to stop). That is right for a single play and wrong for a throne-room: play a Duration NORMALLY first, registering its fx, then use the Way on a LATER play, which physically moves the card out of play. Way of the Butterfly 5 spells out this exact sequence — "if the moved card is a Duration, **its later effects will stop after this turn**, and the Throne Room will be discarded in Clean-up this turn" — and ch. VII REMOVED FROM PLAY lists **Way of the Butterfly / Horse / Turtle** among the six things in the whole game that can remove a Duration. Frog and Chameleon are explicitly NOT among them (Frog 2: a persisting Duration is never discarded, "so Way of the Frog does nothing"; Chameleon 9: "if the played card is a Duration, leave it in play as you normally would"). The fix is still a live "is my card still in play" check on every fx at `_start_of_turn`, touching every Duration in the game — but it is now owed to three shipped Ways × every throne-room × every Duration, not to one corner of one pairing. **This is the row to pay at ph. 11.** Pinned by `test_a_cargo_ship_still_catches_the_card_improve_gained_from_remodelling_it`. |
