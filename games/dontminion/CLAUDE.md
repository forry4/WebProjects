...
| `effects_base.py` … `effects_menagerie.py` (twelve of them) | ONE module per expansion, each owning a disjoint card set |
...
- **THE MAT'S OWN ABILITY IS A KERNEL POOL CONTRIBUTOR** (`_collect_exile_abilities`), the
  ...
  have no `TRIGGERS` entry — but the mat's when-gain ability is CONCURRENT with everything
  else the gain triggered (ch. VI lists it beside Watchtower, Sheepdog and Sleigh), so it must
  arrive through the POOL rather than inline. It is a **yes/no, never a `choose_cards`** —
  the printed mat says "any number", and ch. VII `Your Exile mat 1` settles it the other way:
  "all other copies… **you can't choose to just discard some of them**". It is also **"OTHER
  copies"**, so the card you just gained is never one of them (`Gatekeeper 6`: "your Exile mat
  only allows you to discard 'other copies', meaning not the one you just gained"), which
  falls out of counting what is ON THE MAT when the pool is collected. Gated on a copy
  actually being there, so an ordinary board pays one list lookup.
...
- **GOATHERD — `game["last_turn_trashes"]`, `turn_ctx["trashes"]`'s twin of
  `last_turn_gains`.** A COUNT, because that is all the card asks. It counts trashes **BY**
  that player, not trashes **DURING** their turn — "+1 Card per card the player to your
  right TRASHED on their last turn", and ch. VII Goatherd 2 repeats it ("Goatherd counts how
  many times your right-hand player trashed a card"). Getting it backwards runs the wrong
  way: Bandit, Swindler, Knight and Old Witch are all worded "each other player … trashes",
  so the trasher is the VICTIM, and an attack that makes YOU trash must not pay the
  attacker's Goatherd. **It shipped counting the turn, with a confident comment and no
  citation, and the ph.-10 audit caught it by asking for the source.**
...
| A9 | **Village Green** (Menagerie): does its Reaction require you to REVEAL it, or was the 2020 change reverted? | **Ship the reveal** — "when you discard this other than during Clean-up, you may reveal it to play it". | **The compendium contradicts itself, in the same document.** Ch. V's 2020 errata list is exactly two cards and Village Green is one of them (the change ADDED the reveal); ch. VII Village Green 10 then says "with the new ruling mentioned in 7 above, this change was unnecessary, so Village Green was reverted back to the original version when printed in 2025"; but ch. VIII's timing model, in the SAME document, is headed "Village Green (current version, 2020)" and reads "you may reveal This. **If you do:** Play This" — and the wiki chart's 2025-10-17 capture carries the reveal too. Two sources say reveal, one says reverted, so we took the majority and the more conservative reading (revealing is a cost the player can decline; ours never lets a card do MORE than the strictest source allows). The observable difference is one `reveal()` call. Pinned in `test_cards_menagerie_b.py`. |
| A10 | A card played FROM THE SUPPLY (Overlord, Band of Misfits, an inherited Estate) — is a Way offered for it? | **Only if it is an ATTACK**, which is an asymmetry rather than a choice, and it is pinned in BOTH directions. `_run_supply_ability` emits no `would_resolve`; the attack path reaches the window anyway through `_open_attack_window`'s parked frame. So one Overlord offers a Way for a Militia and not for a Village. | Ch. IV WAYS says "whenever any Action card is played", so the rules want both. **The seam is ph. 8's, not ph. 10's** — Enchantress has the identical asymmetry and it was invisible only because its once-per-turn flag is already spent on the Overlord itself; Ways are the first OBSERVABLE consumer. Closing it needs a new `__play/resolve` routing flag AND a product decision that is genuinely open: an inherited Estate already gets its own offer, so routing the borrowed play through the window too would prompt TWICE for one play. Left as-is deliberately, and pinned by `test_a_way_is_offered_for_a_supply_played_attack_but_not_a_supply_played_action` so the day someone changes it, it is on purpose. |
| A11 | **Way of the Chameleon** on a Vassal / Throne Room: is the card THEY play also swapped? | **Yes** — the swap is turn-wide, with no exception for a card played by a card. | The compendium contradicts itself. Chameleon **5** says "if you play a Vassal, Throne Room or similar using this Way, **the card that it plays is unaffected by this Way**" — but Chameleon **3** and **12** and ch. VIII's own timing model all say the swap covers everything you get THIS TURN, and 12 is decisive about a *different card played later* ("Action–Treasures that give you $ without a '+' … if played in your Buy phase, are still changed"). Ruling 5 cannot be reconciled with 12. We take the turn-wide reading: it is what two rulings plus the timing model say, and it is the one a flat `turn_ctx["chameleon"]` expresses without a special case nothing else in the game needs. Found by the ph.-10 audit, which also noted that ruling 5 was the ONE Chameleon ruling the card's comment block did not cite. Pinned by `test_way_of_the_chameleon_swaps_a_card_played_later_in_the_same_turn`. |
| A12 | **Seize the Day + Outpost**: the player is told they get both turns "as long as you take the Seize the Day turn last" — who chooses the order? | **We do** — the other extra turn is always granted first and Seize's last. | Ch. VII Seize the Day 2 phrases it as a CONDITION rather than an offered choice, and the branch we pick is the only one anybody would take: going the other way strands the Outpost turn (a third in a row, denied by the 2023 errata), so the player would be choosing to throw a turn away. Same reasoning as **B8** — the pool/prompt exists for choices that are genuinely live. If a later card makes the order matter for some other reason, this becomes a real prompt. Pinned by `test_seize_the_day_is_the_exception_that_gives_three_turns_in_a_row` and its single-source control. |
...
| B3 | A **cost read for a "remodel"** should be read at the moment it is used | Develop / Farmland / Trader capture the trashed card's cost **before** the trash resolves | **⚠ THE PREMISE ("nothing in the pool changes costs mid-resolution") IS NOW FALSE.** Menagerie ships three self-costing cards that move with the game state — Destrier (cheaper per card gained this turn), Fisherman (cheaper with an empty discard) and Wayfarer (copies the last gain) — so any on-trash ability that GAINS changes what the trashed card costs, mid-resolution. **Scrap was found doing this and has been moved off the row**: ch. VII Scrap says "first trash, **then check cost**, then resolve the bonuses in the order given", and scrapping a Destrier with a Market Square in hand really does drop it from $6 to $5 and take an option away. Develop, Farmland and Trader have NOT been moved and are now genuinely observable. Found by the ph.-10 audit. |
...
| ~~B9~~ | ~~**The 2025 Duration rule**: "the future effects of a played Duration STOP if the card fails to be in play"~~ | **PAID (post-ph. 10).** An entry whose card is no longer in `in_play` is never PROMOTED at Clean-up | **It was a card-CONSERVATION bug, not the rules divergence this row described for two phases, and that is why it got paid rather than re-recorded.** Play a Caravan normally under a Throne Room (registering its fx), then use Way of the Horse on the SECOND play: the Caravan went back to its pile AND kept a live duration entry, so `_census` counted **12 Caravans on an 11-card pile** and the returned card still drew next turn. The fix is ONE gate, not the kernel-wide `_start_of_turn` check this row predicted — a promoted entry IS the card's accounting (the card leaves `in_play` as it is promoted), so refusing to promote a card that is no longer on the table closes both halves at once. A MULTISET, because zones hold names. It logs `duration_stopped`, since a Duration that simply never pays is indistinguishable from a broken trigger. **It also found a test reaching a state no real play can produce** — `test_champion_survives_the_clean_up_sweep_forever` invoked the on-play effect directly, leaving Champion in no zone at all; both paths that play a card while leaving it in place (Command, Way of the Mouse) exclude Durations for exactly this reason. |
...
| B12 | **The Exile mat cannot join a gain's ability pool LATE** | A when-gain ability that itself Exiles a copy of the gained card does not then re-offer the mat's own discard | `Your Exile mat 3`: "if Bounty Hunter, Camel Train, Displace or Sanctuary is played as the result of a when-gain ability, and you Exile a copy of the gained card, **you may then discard that copy**". The pool is collected once, at the moment the gain is emitted, and the engine has no re-collection mechanism anywhere — adding one means re-opening the concurrent-ability design for a case that needs a when-gain ability to play one of four specific cards. Costs one forgone discard in that corner. Found by the ph.-10 audit. |
| B13 | **An OFF-TURN Way whose payoff lands at the end of the turn or in Clean-up** (Way of the Frog, Way of the Squirrel) | Frog's topdeck never fires; Squirrel refuses off-turn | Both are explicitly ruled TO WORK off-turn (Frog 4 and Squirrel 1 carry the same sentence, "this Way also works if you use it on an opponent's turn", and ch. VI's chart files Squirrel under "at the end of THIS turn", whoever's it is). Neither has a seam: `_end_turn`'s off-turn seat sweep moves `in_play → discard` without emitting `cleanup_discard`, and `turn_ctx["end_draw"]` is a single counter drained for the turn player. Fixing them means an interruptible off-turn sweep and a per-seat `end_draw` — kernel work, scheduled not skipped. **Both now LOG `lost_track` rather than doing nothing**, which was the part that violated an existing rule outright: Frog used to register a watcher that provably could never fire, and a correct-looking trigger that does nothing is indistinguishable from a broken one. Squirrel's stated reason was also wrong ("it evaporates like any off-turn bonus" — that rule is about the per-turn POOLS, and drawing is not one of them, which is why an off-turn reaction can draw at all); the real reason is that there is no per-seat `end_draw` slot. Found by the ph.-10 audit; pinned by four tests in `test_cards_menagerie_b.py`, each with its on-turn control.  **SIZED, and it is not the small job it looked like — scheduled with ph. 11.** The Frog half cannot just add an emit: the off-turn sweep runs INSIDE `_k_cleanup_sweep`, which is already a parked continuation, so a pool parked there would resolve AFTER the watcher expiry, `last_turn_gains`, the extra-turn decision and the new hand draw that follow it in the same function — the ordering trap this file documents four times over. It needs the sweep split into a THIRD parked stage, on the path every turn in every game ends through. The Squirrel half needs a per-seat `end_draw` (a new seat key: SCHEMA bump, fill, migrate case). Ph. 11 (Nocturne) is already scheduled as its own kernel campaign over the turn structure and Clean-up, so both halves land there rather than being built twice. |
