...
same builder that feeds a human — read back into `oddtrick-core`'s `View` by
...
Every position of several complete games, both seats, both auction modes:
    ...
    PYTHONPATH=<repo root> python -m games.oddtrick.tools.gen_view_fixtures \\
        > games/oddtrick/tests/fixtures/views.jsonl

then `cargo test --features bridge` in `rust-cores/oddtrick-core`. The file is
...
from games.oddtrick import bot as B
