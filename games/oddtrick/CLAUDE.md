...
The fixtures are **gitignored**. On a fresh clone that test fails with a message
naming the generator, not a rules break:
...
28-card deck (8–A ×4). 13 cards each: **7 in hand + three 2-card piles**. Only
a pile's top is playable; the card under it becomes playable *and public* when
uncovered. The **middle** pile's bottom is dealt face-up to both; the outer two
are hidden from everyone **including the owner**. Two cards sit out, revealed
at the end.

**Auction.** Opener names level 1–12 and a denomination (4 suits or NT),
committing to score at least that many points. Responses **overtake by +1 or
+2** with a denomination that player has not named. Per-player no-repeat. The
opener may not pass.

**Play.** Declarer leads trick 1. Follow-suit is mandatory **and a pile top
counts as a card you hold**. May ruff when void, never forced to. Winner leads
next.

**Scoring** (contract only; trick points are the yardstick): make → **N²**;
set → defender scores **(N−1) + 4 × shortfall**.
...
* **declarer leads** — the opening lead was measured at **+0.93 pts**, and it
  is the strongest single lever on contract height.
