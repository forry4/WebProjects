"""Self-play calibration for DUMMY mode's server-bot thresholds under CARD
...
`_KONTRA_STRENGTH` were placed with.
    ...
    PYTHONPATH=. python -m games.dissonance.tools.dummy_calibration [rounds]
...
def play_round(rng: random.Random, tally: dict | None = None) -> dict:
    g = E.new_game(["a", "b"], rng, opener=rng.randrange(2), mode="dummy")
    guard = 0
    while g["phase"] != "over":
        seat = E.turn_seat(g)
        if seat is None:
...
def sweep(n: int, seed: int = 7) -> None:
    ...
    for _ in range(n):
        ...
        probe = E.new_game(["a", "b"], rng, mode="dummy")
    ...
    mean_pay = sum(pay) / n
    target = E.MATCH_TARGET["dummy"]
    print(f"\n== dummy / card scoring  ({n} rounds) ==")
