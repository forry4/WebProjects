...
from games.spender_duel import main


def _seed(tmp_path, monkeypatch):
    monkeypatch.setattr(coredb, "DB_PATH", str(tmp_path / "duel_hist.db"))
...
def _finished_rows(n):
    ...
    state = json.dumps({
        "players": {"a": "A", "b": "B"}, "status": "over",
        "game": {"players": {"a": {"points": 20}, "b": {"points": 5}}, "winner": "a"},
    })
    now = int(time.time())
    conn = coredb.get_db_conn()
    cur = conn.cursor()
