...
**Verdict — NOT a ceiling, a self-play PLATEAU (do-not-relitigate the framing).** The three self-play levers (data/capacity/policy) plateaued *because self-play cannot generate what no self-play agent demonstrates* (#8's patient development). A below-average human beats v2 ~70% of completed games → it is nowhere near a ceiling (no AI is, short of a solved game). The live lever is **human-game debias on the diagnosed under-development blind spot**; imitation is OFF the table (below-average demonstrator). The 12k net-strength corpus is KEPT at `C:/Users/Forrest/duel_run/v3/`. Session tooling (native, `--features bridge`): `attn_bench`, `gate_sims`/`gate_rollout`, `harvest_attn`/`harvest_pv` (attn leaf), `gate_netleaf`, `train_attn`; analysis: `scratchpad/duel_db.py` + `duel_trace.py`.
