...
mod shipped_terms {
    ...
    /// PINNED TO THE ENGINE'S OWN OUTPUT, mechanically.
    ///
    /// `shipped_terms.jsonl` is generated by `tools/gen_shipped_terms.py`
    /// straight out of `engine._terms_for`, so this tracks a re-pricing instead
    /// of having to be remembered. Deriving the expectation from
    /// `shipped_classic_terms`' own arithmetic would make the test agree with
    /// any bug in it, which is exactly how three bins stayed wrong while each
    /// was internally consistent.
    ///
    /// Regenerate after any scoring change:
    ///   PYTHONPATH=<root> python -m games.dissonance.tools.gen_shipped_terms \
    ...
    fn the_shipped_terms_match_the_engine() {
        ...
        let txt = std::fs::read_to_string(path).unwrap_or_else(|e| {
            panic!("{path}: {e}\nregenerate with tools/gen_shipped_terms.py")
        });
        let mut checked = 0;
        for line in txt.lines().filter(|l| !l.trim().is_empty()) {
            let v: serde_json::Value = serde_json::from_str(line).expect("fixture row parses");
            let n = |k: &str| v[k].as_i64().expect(k) as i32;
            // The fixture carries the jump axis too; `shipped_classic_terms` is
            // the plain undoubled no-jump contract, so only those rows apply to
            // it. The others are here so a jump-rate change is still loud.
            if n("jump") != 0 {
                continue;
            }
            let level = n("level");
            let c = shipped_classic_terms(level, 0);
            assert_eq!(c.make_base, n("make"), "level {level}: make base");
            assert_eq!(c.set_base, n("set_base"), "level {level}: set base");
            assert_eq!(c.over, n("over"), "level {level}: overtrick rate");
            assert_eq!(c.short, n("short"), "level {level}: shortfall rate");
            assert_eq!(c.null, Some(n("null")), "level {level}: null consolation");
            checked += 1;
        }
        assert!(checked >= 8, "only {checked} plain levels in the fixture");
    }

    /// THE JUMP RATE IS IN THE FIXTURE TOO, and this is what would have caught
    /// the documentation half of the same drift: the manual's own section
    /// defining the rule said `JUMP_SET_BONUS` was 3 for three days after
    /// `a317bb1` shipped 6.
    #[test]
    #[cfg(feature = "bridge")]
    fn the_jump_rate_is_what_the_engine_charges() {
        let path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../games/dissonance/tests/fixtures/shipped_terms.jsonl"
        );
        let txt = std::fs::read_to_string(path).expect("fixture");
        let mut rates = Vec::new();
        for line in txt.lines().filter(|l| !l.trim().is_empty()) {
            let v: serde_json::Value = serde_json::from_str(line).unwrap();
            let n = |k: &str| v[k].as_i64().unwrap() as i32;
            if n("jump") == 0 {
                continue;
            }
            // set_base(jump j) - set_base(0) must be exactly rate * j.
            let plain = shipped_classic_terms(n("level"), 0).set_base;
            let j = n("jump");
            assert_eq!((n("set_base") - plain) % j, 0, "jump bonus is not linear in j");
            rates.push((n("set_base") - plain) / j);
        }
        assert!(!rates.is_empty(), "the fixture carries no jump rows");
        let first = rates[0];
        assert!(rates.iter().all(|&r| r == first), "the jump rate is not constant: {rates:?}");
        assert_eq!(first, 6, "JUMP_SET_BONUS moved -- regenerate and update the manual");
    }
