...
use dissonance::auction::{eval_hand, denom_str, HandEval, MAX_LEVEL, NDEN, NULL_DENOM};
...
fn pts_in(dd: &mut Dd, base: &dissonance::state::State, denom: u8, decl: usize,
          declarer_leads: bool) -> i32 {
    let s = dissonance::state::State {
        trump: denom,
        trick: 0,
        led: -1,
...
fn resolve_declarer(
    ...
    rng: &mut Rng,
    tk: usize,
) -> (Decl, dissonance::state::State, i32, bool, f64) {
    ...
    let mut buf = Vec::new();
    let blind_worlds: Vec<dissonance::state::State> =
        (0..tk).map(|_| v_blind.determinize(rng, &mut buf)).collect();
    let look_worlds: Vec<dissonance::state::State> =
    ...
    for (is_hand, take, give) in cands {
        ...
        for d in declarable(cfg, bid, is_hand) {
            ...
            let mut vals: Vec<i32> = (0..worlds.len())
                ...
                .collect();
            let v = dissonance::skat::quantile(&mut vals, cfg.q);
    ...
    let believed = if d.is_null() {
        ...
        let mut v: Vec<i32> = worlds
            ...
            .collect();
        dissonance::skat::quantile(&mut v, cfg.q)
    };
