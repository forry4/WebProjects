...
def main():
    ...
    ap.add_argument("--shape-a", type=float, default=SHAPE_A,
                    help="value-target margin-shaping weight (lower -> purer win/loss)")
    ap.add_argument("--beta", type=float, default=BETA,
                    help="value-target weight on the rollout root value (0 -> pure ground-truth)")
    args = ap.parse_args()
    # Experiment override: value target = (1-BETA)*[(1-SHAPE_A)*(2y-1) +
    # SHAPE_A*tanh(margin/scale)] + BETA*rootv. --beta 0 removes the rollout
    # anchoring; lowering --shape-a pushes toward pure win/loss (2y-1).
    SHAPE_A, BETA = args.shape_a, args.beta
    print(f"value target: SHAPE_A={SHAPE_A} BETA={BETA} "
          f"(outcome {(1 - BETA) * (1 - SHAPE_A):.2f}, "
          f"margin {(1 - BETA) * SHAPE_A:.2f}, rootv {BETA:.2f})")
