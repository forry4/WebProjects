...
//!
//! `games/oddtrick/engine.py::view_for` already builds exactly the information
...
// `games/oddtrick/tools/gen_view_fixtures.py` whenever `view_for` changes shape.
#[cfg(test)]
mod payoff_parity {
    ...
    //
    // Regenerate with `games/oddtrick/tools/gen_payoff_fixtures.py`.
    use super::*;

    fn rows() -> Vec<Value> {
        let path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../games/oddtrick/tests/fixtures/payoff.jsonl"
        );
        let text = std::fs::read_to_string(path).unwrap_or_else(|e| {
            panic!("{path}: {e}\nRegenerate with:\n  PYTHONPATH=<repo root> python -m \
                    games.oddtrick.tools.gen_payoff_fixtures > \
                    games/oddtrick/tests/fixtures/payoff.jsonl")
        });
        text.lines()
...
mod fixture_replay {
    ...
    fn fixtures() -> Vec<Value> {
        // `CARGO_MANIFEST_DIR` is rust-cores/oddtrick-core.
        let path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../games/oddtrick/tests/fixtures/views.jsonl"
        );
        let text = std::fs::read_to_string(path).unwrap_or_else(|e| {
            panic!(
                "{path}: {e}\nRegenerate with:\n  PYTHONPATH=<repo root> python -m \
                 games.oddtrick.tools.gen_view_fixtures > \
                 games/oddtrick/tests/fixtures/views.jsonl"
            )
        });
        text.lines()
