...
use duel_core::mcts::{pick, root_search, Opts, RngShuffler, RootStats};
...
fn play_game(
    ...
    loop {
        ...
        let opts = Opts { max_iters: Some(sims), time_limit: Some(f64::INFINITY), ..Default::default() };
        let stats = match root_search(&st, mover, "hard", &opts, &mut rng) {
            Some(s) => s,
...
fn main() {
    ...
    for g in 0..args.games {
        ...
        let rows = play_game(g, args.sims, args.temp_plies, args.temp, args.target_temp, gseed, args.cap);
        if rows.is_empty() {
            discarded_games += 1;
            continue;
        }
